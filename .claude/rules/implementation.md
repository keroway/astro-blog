...
C5. **Node は v22.12.0 以上 (Astro 7 の要件)。運用は 24 にピン。** 環境ごとの担い手は分かれる — ローカルは `mise.toml` (node 24 / pnpm 11)、CI は `actions/setup-node` の `node-version: "24"`、Vercel は `package.json` の `packageManager` (corepack 経由)。この 3 箇所すべてを 24 系に揃える。
