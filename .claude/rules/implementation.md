...
E1. **`pnpm run lint`** — Biome lint + format check (= `biome ci`。CI の `lint` ジョブは加えて `pnpm run lint:alt`・`pnpm run lint:tokens-doc`・`pnpm run lint:cms-config`・`pnpm run lint:description-length` も走る)。差分が出ない状態にする。安全な自動修正だけ試したいときは `pnpm exec biome check --write .` を別途実行する (`--unsafe` は付けない)。
...
F4. **失敗した CI は共有コマンド `/fix-pr <PR番号>` で当たる。** PR 番号は明示的に渡す (誤爆防止。省略すると現在ブランチの PR を自動検出するが、このリポジトリでは省略しない)。
