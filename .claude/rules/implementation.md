...
C1. **`getStaticPaths()` の `params` には `post.id` をエンコードせずに渡す。** Astro 6 の Content Layer API は内部でエンコードする。エンコードすると二重デコードで 404。
...
C5. **Node は v22.12.0 以上。** Astro 6 の要件。CI / Vercel / ローカルすべてここに揃える。
...
D2. **色・余白・角丸は `src/styles/global.css` の CSS 変数を使う。** 新トークンを増やすときは既存の命名規則 (`--accent`, `--surface`, `--gray`) に揃える。
...
E1. **`pnpm run lint`** — Biome lint + format check (= `biome ci`、CI の `lint` ジョブと完全一致)。差分が出ない状態にする。安全な自動修正だけ試したいときは `pnpm exec biome check --write .` を別途実行する (`--unsafe` は付けない)。
E2. **`pnpm exec astro check`** — typecheck (CI の `typecheck` ジョブと一致)。
E3. **`pnpm exec astro build`** — schema 違反と build 時エラーを surface (CI の `build` ジョブと一致、`pnpm run build` ではなく `astro build` 直叩き)。
E4. **`pnpm exec playwright test`** — E2E。ポート競合のときは `PLAYWRIGHT_PORT=4322` で。
E5. **目視確認** — 1280 / 768 / 375 の 3 解像度で実画面を確認。

`/ship-check` で E1〜E4 を順に走らせられる。**スキップ厳禁。**
