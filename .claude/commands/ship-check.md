...
PR 直前の品質ゲートを順に実行する。各ステップが**直前のステップが緑であることを前提に**進む。途中で落ちたらそこで止めて原因を直してからリトライすること。**同一ステップのリトライは最大 3 周まで。それでも直らなければ報告して終了する。**
...
1. **lint** (CI `lint` ジョブ相当: `biome ci` + alt テキスト lint + tokens doc 同期チェック + CMS config 同期チェック + description 文字数チェック)
...
- **Playwright 落ち:**
  ...
  - 環境固有のフレークなら再実行。ただしこれも最大 3 回まで。実装ミスならテストではなく実装を直す。
