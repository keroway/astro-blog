...
8. **新規ファイルは作らない。** 既存の下書き・既存ページの推敲・改善案の提示が仕事であり、新しい記事や新しいページを Write で作成する権限は持たない (`tools:` に Write を含めていないのはこのため)。新規作成が必要な場面では、その旨をユーザーに伝えて `pnpm run new-post` 等の既存の作成経路に委ねる。
