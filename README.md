...
push / PR をトリガーに GitHub Actions（[`.github/workflows/ci.yml`](./.github/workflows/ci.yml)）で 7 ジョブを実行します。`pnpm run build`（= `astro check && astro build`）の typecheck と build を別ジョブに分割し、PR 上で失敗を早く surface する設計です。Build ジョブが生成した `dist/` を artifact として E2E / Lighthouse / Link check の 3 ジョブが再利用します。
