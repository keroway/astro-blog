...
| alt テキスト lint | `pnpm run lint:alt` | `src/content/{blog,works}` の markdown 画像で alt が空/短すぎる/プレースホルダ/数値のみ、または `imgur` / `googleusercontent` の外部画像参照を検出（CI の lint ジョブでも実行） |
...
- [0002 — CMS: Keystatic 採用](./docs/adr/0002-cms.md)（Deprecated / Superseded by 0016）
...
- [0013 — Web フォント自己ホスト化](./docs/adr/0013-web-fonts-self-hosting.md)（Accepted）
- [0014 — Keystatic 管理 UI の Google Fonts 参照除去](./docs/adr/0014-keystatic-google-fonts-removal.md)（Accepted）
- [0015 — 全文検索基盤（Pagefind）](./docs/adr/0015-pagefind-search-platform.md)（Accepted）
- [0017 — Astro 7 へのメジャーアップグレード](./docs/adr/0017-astro-7-upgrade.md)（Accepted）
- [0018 — コンテンツ画像の astro:assets パイプライン移行](./docs/adr/0018-astro-assets-media-pipeline.md)（Accepted）
- [0019 — 記事フィードバックチャネル](./docs/adr/0019-feedback-channel.md)（Accepted）
...
push / PR をトリガーに GitHub Actions（[`.github/workflows/ci.yml`](./.github/workflows/ci.yml)）で 7 ジョブを実行します。ローカルの `pnpm run build`（= `astro check && astro build`）に相当する処理を、CI では `typecheck` ジョブの `astro check` と `build` ジョブの `astro build` 直叩きに分割し、PR 上で失敗を早く surface する設計です。Build ジョブが生成した `dist/` を artifact として E2E / Lighthouse / Link check の 3 ジョブが再利用します。

```mermaid
flowchart TD
    P["push / PR"] --> CI["GitHub Actions CI"]
    CI --> L["Lint<br/>Biome + lint:alt"]
    CI --> U["Unit tests<br/>vitest"]
    CI --> T["Typecheck<br/>astro check"]
    CI --> B["Build<br/>astro build"]
    B --> E["E2E<br/>Playwright（chromium / firefox / mobile）"]
    B --> LH["Lighthouse CI"]
    B --> LC["Link check<br/>lychee"]
    L --> M["main へマージ"]
    U --> M
    T --> M
    E --> M
