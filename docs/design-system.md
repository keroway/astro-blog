...
| `--kw-road` | `#385D7E` | `#A0B7D0` | 東海道の道筋 |
| `--kw-road-soft` | `rgba(56,93,126,0.22)` | `rgba(160,183,208,0.24)` | 道筋の太い下地 |
...
| トークン | `--kw-road` / `--kw-road-soft` を `src/styles/tokens.css` にライト/ダーク両値で定義済み。地点（宿場）は `--kw-accent-strong` と `--kw-sand` を重ねる |
| 装飾実装（`HeroBackdrop`） | `src/components/HeroBackdrop.astro` が富士・道筋・地点・水面・地名ラベルを 1 枚のインライン SVG（`viewBox="0 0 1200 700"`、`preserveAspectRatio="none"`）に統合する。道筋は太い下地 (`--kw-road-soft`) と細い破線 (`--kw-road`) を重ねる。全体 `aria-hidden`・静止（モーションなし）・`position: absolute; inset: 0` で CLS なし。モバイルでは地名ラベルと水面を隠して簡略化する |
