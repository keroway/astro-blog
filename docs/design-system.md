...
| 富士の遠景 | 東海道程ヶ谷 | 葛飾北斎 | 『富嶽三十六景』 | 実装済み（`HeroBackdrop`） |
| 松並木 | 東海道程ヶ谷 | 葛飾北斎 | 『富嶽三十六景』 | 実装済み（`.kw-divider-pine`） |
...
> - **§9.6 道筋 (`--kw-road-*`)**: **命名予約のみで `:root` 未追加**。道筋は専用色を持たず、既存トークン（`--kw-fuji` / `--kw-sand` / `--kw-rule-soft`）で描く。専用色が必要になった時点で実体化する。
...
| 装飾実装 | `global.css` の `.kw-divider-pine`。`conic-gradient` で松並木のシルエットを横一列に繰り返す水平区切り（高さ 20px）。色は `--kw-pine-soft` + `--kw-pine`。ホーム HERO 直下のセクション区切りで使用 |
...
| 装飾実装 | `HeroBackdrop.astro` のインライン SVG `<polygon>`（富士の稜線、`fill: --kw-fuji`、`opacity: 0.06`）。道筋・地点・水面と同一 SVG の座標系に統合してホーム HERO 背景に敷く（旧 `.kw-watermark-fuji` を統合）。`aria-hidden`・静止 |

### 9.6 道筋（街道）

東海道の街道そのものを表す語彙です（→ [ADR 0011](./adr/0011-motif-road-path.md)）。波・松並木・富士の遠景と同じ「横浜近郊という地理的近接」の軸に属し、新しい出典を足すのではなく、既存の地理的物語に街道という連結線を一本引く位置づけです。**新色を足さず、既存トークン（`--kw-rule` / `--kw-fuji` / `--kw-sand`）の範囲で描きます。** ホーム HERO 背景で `HeroBackdrop.astro`（富士の遠景と同一 SVG に統合）として実装済みです。
...
| トークン | `--kw-road-*` は命名予約のみで `:root` 未追加。専用色を持たず、線は `--kw-fuji`（`opacity` 0.22）・地点（宿場）は既存アクセント `--kw-sand`（`opacity` 0.5）・水面は `--kw-rule-soft` で描く。専用色が必要になった時点で `--kw-road` / `--kw-road-soft` を実体化する |
| 装飾実装（`HeroBackdrop`） | `src/components/HeroBackdrop.astro` が富士・道筋・地点・水面を 1 枚のインライン SVG（`viewBox="0 0 1200 700"`、`preserveAspectRatio="none"`）に統合する。(a) **道筋ライン** — 左上→右下へ抜ける一本の `<path>`（`stroke: --kw-fuji`、`fill: none`、`vector-effect: non-scaling-stroke`）。(b) **宿場の地点** — 道筋上の `<circle>` 4 個（`fill: --kw-sand`）。(c) **水面の水平線** — 下部の薄い `<line>` 2 本（`stroke: --kw-rule-soft`）。全体 `aria-hidden`・静止（モーションなし）・`position: absolute; inset: 0` で CLS なし。`@media (max-width: 900px)` で道筋・地点・水面を隠し富士のみに簡略化する |
