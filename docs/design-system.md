...
| モチーフ | 画題 | 作者 | シリーズ | 図像化 |
|---|---|---|---|---|
| 波 | 神奈川沖浪裏 | 葛飾北斎 | 『富嶽三十六景』 | 実装済み（`.kw-wave`） |
| 富士の遠景 | 東海道程ヶ谷 | 葛飾北斎 | 『富嶽三十六景』 | 設計のみ（実装は follow-up） |
| 松並木 | 東海道程ヶ谷 | 葛飾北斎 | 『富嶽三十六景』 | 設計のみ（実装は follow-up） |
| 道筋（街道） | 戸塚・神奈川・程ヶ谷の街道 | 歌川広重 / 葛飾北斎 | 『東海道五十三次』『富嶽三十六景』 | 設計のみ（実装は follow-up #242） |
...
| 既存パレット非衝突 | 紺碧（`--kw-navy`）・白浪（`--kw-paper`）・砂金（`--kw-sand`）の三色構成を壊さない。新色は彩度を抑えて既存トーンに馴染ませる |
...
> **注:** 9.3〜9.5 の「想定トークン」は設計上の予約であり、現時点で `:root` には未追加です。実装 Issue で値を確定します。命名は §4.2 の `--kw-{category}-{scale}` 規則に従います。
...
| 想定トークン | 既存。`currentColor`（`--kw-fg-link`）+ `opacity: 0.85`。専用色トークンは持たない |
| 実装 | `global.css` の `.kw-wave`（幅 140px・高さ 12px、`radial-gradient` 5 連弧）。使用箇所はホームヒーロー右端・About マストヘッド右端・フッター左下（→ §7.4 Monogram） |
...
| 想定トークン命名 | `--kw-pine-*`（松 = 緑系）。彩度を抑えた深緑〜苔色を想定。例: `--kw-pine`（プリミティブ）/ `--kw-pine-soft`（薄掛け）。セマンティック層を設ける場合は `--kw-divider-pine` 等 |
| 装飾実装の具体候補 | (a) **松並木ディバイダ** — 横一列に並ぶ松のシルエットを `repeating-linear-gradient` か小さな SVG パスのリピートで描く水平区切り線。(b) 高さ 2〜3px の細い罫線の上に等間隔の三角形（松の樹形）を `clip-path` か `mask` で載せる。色は `--kw-pine-soft` で控えめに。ライト/ダーク両方で `currentColor` 基準にすると破綻しにくい |
...
| 想定トークン命名 | `--kw-fuji-*`（富士の遠景 = 霞んだ青/藍系）。彩度を落とした霞んだ藍を想定。例: `--kw-fuji`（プリミティブ）/ `--kw-fuji-faint`（ウォーターマーク用の極薄）。既存の `--kw-navy` より明度高め・彩度低めで、遠景の空気感を出す |
| 装飾実装の具体候補 | (a) **富士の薄掛けウォーターマーク** — 富士の稜線（左右対称の台形 + 山頂の凹み）を `clip-path: polygon()` かインライン SVG で描き、`opacity` を 0.04〜0.08 程度に落としてヒーローや余白の背景に敷く。(b) 稜線だけを 1px の薄い線（`--kw-fuji-faint`）で引く「線画」バリエーション。いずれも `aria-hidden`、`prefers-reduced-motion` 下では動かさない（そもそも静止が基本） |

### 9.6 道筋（街道）

東海道の街道そのものを表す語彙です（→ [ADR 0011](./adr/0011-motif-road-path.md)）。波・松並木・富士の遠景と同じ「横浜近郊という地理的近接」の軸に属し、新しい出典を足すのではなく、既存の地理的物語に街道という連結線を一本引く位置づけです。**新色を足さず、既存トークン（`--kw-rule` / `--kw-fuji` / `--kw-sand`）の範囲で描きます。**

| 項目 | 内容 |
|---|---|
| 意味 | 程ヶ谷・戸塚・神奈川宿を結ぶ東海道の道筋。移動・記録・中継点を象徴する。「旅人が宿場を通過しながら記録を残す」というサイトコンセプト（横浜で技術の手控えを残す）に直結する語彙 |
| 用途指針 | HERO 背景で富士の遠景に重ね、画面を斜めに抜ける一本の道として敷く。富士と同じく「ごく薄いウォーターマーク」として扱い、前景テキスト・カードの可読性を損なわない。1 画面に 1 本まで。本文区切りには使わない |
| 想定トークン命名 | `--kw-road-*`（命名予約のみ）。当面は専用色を持たず、線は `--kw-rule` 系・遠景の道は `--kw-fuji`・地点（宿場）は既存アクセント `--kw-sand` で描く。専用色が必要になった時点で `--kw-road` / `--kw-road-soft` を実体化する |
| 装飾実装の具体候補 | (a) **道筋ライン** — 左上→右下へゆるく抜ける一本の曲線パスをインライン SVG の `<path>`（`stroke` に `--kw-fuji` か `--kw-rule-strong`、`fill: none`、低 `opacity`）で描く。(b) **宿場の地点** — 道筋上に小さな `<circle>` を 3〜4 個（`fill: --kw-sand`、半径小、低 `opacity`）置き、宿場・中継点を示す。(c) **水面の水平線** — 下部にごく薄い水平 `<line>`（`stroke: --kw-rule-soft`）を 1〜2 本引き、横浜の海を示唆する。いずれも `aria-hidden`、静止（モーションなし）が基本。富士・道筋・地点・水面を同一 SVG の座標系（`viewBox`）で描くとモチーフ同士が整合する |
