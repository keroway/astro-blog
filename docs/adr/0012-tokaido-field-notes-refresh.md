...
3. **道筋を主装飾へ引き上げる**: `--kw-road` / `--kw-road-soft` を追加し、HERO の道筋・地点・水面を背景装飾として扱う。地名ラベルは置かず、おおよその旧東海道ルートと宿場プロットだけで雰囲気を出す。ただし装飾は `aria-hidden` のまま情報を持たせない。
...
既存の Kanagawa デザインシステムを捨てずに、東海道の道筋・海岸線・朱印・紙面・技術手控えの語彙を前面へ出せる。ADR 0011 の保守的な判断は「道筋追加時の最小変更」として残し、今回の視覚刷新では ADR 0012 が優先される。
