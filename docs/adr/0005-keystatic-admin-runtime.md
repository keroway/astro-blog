...
- `astro.config.mjs`
  ...
  - integration mount は `VERCEL_ENV` で 3 経路:
    - **Production**: `react() + keystatic()` を mount + `PUBLIC_KEYSTATIC_STORAGE_KIND=github` を強制
    - **Preview**: Keystatic 統合を **mount しない**（`/keystatic` は 404）。Preview の Vercel Function も ephemeral filesystem なので local モードで起動するとデータロストになるため、編集を完全に禁止する設計
    - **Local dev** (`VERCEL_ENV` 未定義): mount + local mode で従来通り動作
- `keystatic.config.ts`
  - `PUBLIC_KEYSTATIC_STORAGE_KIND` 環境変数で `local` / `github` を切替 (config はブラウザにも bundle されるため `import.meta.env.PUBLIC_*` 経由でアクセスする必要があり、`PUBLIC_` プレフィックス必須)
  - 本番 (`github`) では branch storage を使用し、`keystatic/<slug>` ブランチへ commit → main へ PR
  - **fail-fast ガード**: `VERCEL_ENV === "production"` のとき `PUBLIC_KEYSTATIC_STORAGE_KIND=github` を強制する。env 未設定で local モードに落ちると Vercel Function の ephemeral filesystem に書き込もうとして Admin UI が無音で機能不全になるため、ビルド時に明確なエラーで止める方針 (fail-open ではなく fail-closed)。チェック自体は `astro.config.mjs` (サーバー専用) 側に集約。
...
- ローカルで `PUBLIC_KEYSTATIC_STORAGE_KIND=local` (または未設定) のまま `pnpm run dev` → `/keystatic` で従来通り動くこと
