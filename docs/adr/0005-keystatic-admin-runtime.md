...
- `astro.config.mjs`
  ...
  - integration mount は `VERCEL_ENV` で 3 経路:
    - **Production**: `react() + keystatic()` を mount + `KEYSTATIC_STORAGE_KIND=github` を強制
...
- `keystatic.config.ts`
  ...
  - **fail-fast ガード**: `VERCEL_ENV === "production"` のとき `KEYSTATIC_STORAGE_KIND=github` を強制する。env 未設定で local モードに落ちると Vercel Function の ephemeral filesystem に書き込もうとして Admin UI が無音で機能不全になるため、ビルド時に明確なエラーで止める方針 (fail-open ではなく fail-closed)
