...
着手のきっかけは AI コーディング支援の発達です。これなら形にできそうだと開発に踏み切りました。実在の人物データの取得元として Wikidata を使うというアイデアは、開発を進める中で AI から提案されたもので、そこから「QID を指定して歴史データを取り込む」という現在の中心機能へと広がっていきました。身近な人々の年表という出発点と、Wikidata から引いてくる歴史上の人物の年表が、同じ仕組みの上で扱えるようになっています。
...
- **設計の多くは AI の提案に委ねた。** クレート分割や IR の構成といった構造的な判断は、AI から提示される選択肢に妥当な根拠が添えられていることが多く、内容を理解したうえで了承する、という進め方が中心でした。自分一人では踏み込めなかった設計に手が届いた一方で、提案を評価して受け入れる側のリテラシーが問われるのだと感じています。
- **WebAssembly 開発は初めての経験でした。** Rust で書いたコンパイラをブラウザで動かすために `wasm-bindgen` でラッパーを用意する過程は、ビルドターゲットの切り替えや JavaScript とのインターフェース設計など、勘所のひとつひとつが学びになりました。
- **各種レジストリへの公開も初挑戦でした。** npm（`@keroway/tdsl-wasm`）・VS Code 拡張・Homebrew といった配布チャネルへの登録はどれも未経験で、それぞれの公開フローや必要なメタデータ・設定の仕組みを一通り通すこと自体が、地味ながら大きな収穫でした。
