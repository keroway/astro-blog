...
const categories = [
  ...new Set(rows.map((r) => r.category).filter((c) => c !== "（未設定）")),
].sort();
...
const lines: string[] = [
  ...
  ...rows.map(
    (r) =>
      `| ${r.slug} | ${r.title} | ${r.pubDate} | ${r.category} | ${r.hasHero} | ${r.assessment} |`
  ),
];
...
console.log(
  `  公開維持: ${summary.公開維持}, 更新必要: ${summary.更新必要}, アーカイブ: ${summary.アーカイブ}`
);
